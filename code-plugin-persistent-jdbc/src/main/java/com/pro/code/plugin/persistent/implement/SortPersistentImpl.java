...
public class SortPersistentImpl extends ToolPersistent implements ISortPersistent {
  ...
  public static final String VIRTUAL_COLUMN_DT_NAME = "( SELECT dt.DT_NAME FROM PRO_DT dt WHERE dt.DT_ID IS NOT NULL AND dt.DT_ID = sort.DT_ID ) AS DT_NAME";
...
//  }
  ...
  public SortVO getSortVOByPk(java.lang.String sortId) throws CodePluginException {
    if (log.isDebugEnabled()) {
      log.debug("Staring call SortPersistent.getSortVOByPk ");
      log.debug("parameter sortId is : " + sortId);
    }
    try {
      if (ToolUtil.isNullStr(sortId)) {
  ...
  public Collection<SortVO> getAllSortVO() throws CodePluginException {
    if (log.isDebugEnabled()) {
      log.debug("Staring call SortPersistent.getAllSortVO ");
    }
    try {
      StringBuilder querySql = new StringBuilder(SELECT_VO_SQL);
      querySql.append(this.getSortSql(SORT, COLUMNS_PARAMETER, TABLE_ALIAS));
  ...
  public Paging<SortVO> pagingGetSortVO(Parameter parameter) throws CodePluginException {
    if (log.isDebugEnabled()) {
      log.debug("Staring call SortPersistent.pagingGetSortVO ");
      log.debug("parameter parameter is : " + parameter);
    }
    try {
      if (parameter == null) {
      ...
      Paging<SortVO> paging = new Paging<>(parameter);
      StringBuilder countSql = new StringBuilder(COUNT_SQL);
      Long count = this.queryCount(countSql, Long.class);
      paging.setCount(count);
      if (count > 0) {
        ...
        if (ToolUtil.isNotEmpty(sortSet)) {
          ...
          SortQuery sortQuery = new SortQuery();
          sortQuery.setSortIdAndin(new ArrayList<>(inSortId));
          Collection<SortVO> rSortVOSet = this.querySortVO(sortQuery);
  ...
  public Collection<SortVO> querySortVO(SortQuery sortQuery) throws CodePluginException {
    if (log.isDebugEnabled()) {
      log.debug("Staring call SortPersistent.querySortVO ");
      log.debug("parameter sortQuery is : " + sortQuery);
    }
    try {
      StringBuilder querySql = new StringBuilder(SELECT_VO_SQL);
      ...
      querySql.append(this.getSortSql(SORT, COLUMNS_PARAMETER, TABLE_ALIAS));
      return this.query(querySql, SortVO.class, sortQuery);
    } catch (org.springframework.dao.DataAccessException e) {
      if (log.isErrorEnabled()) {
        log.error(e.getMessage(), e);
      }
  ...
  public Paging<SortVO> pagingQuerySortVO(Parameter parameter, SortQuery sortQuery) throws CodePluginException {
    if (log.isDebugEnabled()) {
      log.debug("Staring call SortPersistent.pagingQuerySortVO ");
      log.debug("parameter parameter is : " + parameter);
      log.debug("parameter sortQuery is : " + sortQuery);
    }
    try {
