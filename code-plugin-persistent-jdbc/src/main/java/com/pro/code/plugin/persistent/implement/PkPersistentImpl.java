...
//  }
  ...
  public PkVO getPkVOByPk(java.lang.String pkId) throws CodePluginException {
    if (log.isDebugEnabled()) {
      log.debug("Staring call PkPersistent.getPkVOByPk ");
      log.debug("parameter pkId is : " + pkId);
    }
    try {
      if (ToolUtil.isNullStr(pkId)) {
  ...
  public Collection<PkVO> getAllPkVO() throws CodePluginException {
    if (log.isDebugEnabled()) {
      log.debug("Staring call PkPersistent.getAllPkVO ");
    }
    try {
      StringBuilder querySql = new StringBuilder(SELECT_VO_SQL);
      querySql.append(this.getSortSql(SORT, COLUMNS_PARAMETER, TABLE_ALIAS));
  ...
  public Paging<PkVO> pagingGetPkVO(Parameter parameter) throws CodePluginException {
    if (log.isDebugEnabled()) {
      log.debug("Staring call PkPersistent.pagingGetPkVO ");
      log.debug("parameter parameter is : " + parameter);
    }
    try {
      if (parameter == null) {
      ...
      Paging<PkVO> paging = new Paging<>(parameter);
      StringBuilder countSql = new StringBuilder(COUNT_SQL);
      Long count = this.queryCount(countSql, Long.class);
      paging.setCount(count);
      if (count > 0) {
        ...
        if (ToolUtil.isNotEmpty(pkSet)) {
          ...
          PkQuery pkQuery = new PkQuery();
          pkQuery.setPkIdAndin(new ArrayList<>(inPkId));
          Collection<PkVO> rPkVOSet = this.queryPkVO(pkQuery);
  ...
  public Collection<PkVO> queryPkVO(PkQuery pkQuery) throws CodePluginException {
    if (log.isDebugEnabled()) {
      log.debug("Staring call PkPersistent.queryPkVO ");
      log.debug("parameter pkQuery is : " + pkQuery);
    }
    try {
      StringBuilder querySql = new StringBuilder(SELECT_VO_SQL);
      ...
      querySql.append(this.getSortSql(SORT, COLUMNS_PARAMETER, TABLE_ALIAS));
      return this.query(querySql, PkVO.class, pkQuery);
    } catch (org.springframework.dao.DataAccessException e) {
      if (log.isErrorEnabled()) {
        log.error(e.getMessage(), e);
      }
  ...
  public Paging<PkVO> pagingQueryPkVO(Parameter parameter, PkQuery pkQuery) throws CodePluginException {
    if (log.isDebugEnabled()) {
      log.debug("Staring call PkPersistent.pagingQueryPkVO ");
      log.debug("parameter parameter is : " + parameter);
      log.debug("parameter pkQuery is : " + pkQuery);
    }
    try {
