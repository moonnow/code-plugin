...
//  }
  ...
  public DtVO getDtVOByPk(java.lang.String dtId) throws CodePluginException {
    if (log.isDebugEnabled()) {
      log.debug("Staring call DtPersistent.getDtVOByPk ");
      log.debug("parameter dtId is : " + dtId);
    }
    try {
      if (ToolUtil.isNullStr(dtId)) {
  ...
  public Collection<DtVO> getAllDtVO() throws CodePluginException {
    if (log.isDebugEnabled()) {
      log.debug("Staring call DtPersistent.getAllDtVO ");
    }
    try {
      StringBuilder querySql = new StringBuilder(SELECT_VO_SQL);
      querySql.append(this.getSortSql(SORT, COLUMNS_PARAMETER, TABLE_ALIAS));
  ...
  public Paging<DtVO> pagingGetDtVO(Parameter parameter) throws CodePluginException {
    if (log.isDebugEnabled()) {
      log.debug("Staring call DtPersistent.pagingGetDtVO ");
      log.debug("parameter parameter is : " + parameter);
    }
    try {
      if (parameter == null) {
      ...
      Paging<DtVO> paging = new Paging<>(parameter);
      StringBuilder countSql = new StringBuilder(COUNT_SQL);
      Long count = this.queryCount(countSql, Long.class);
      paging.setCount(count);
      if (count > 0) {
        ...
        if (ToolUtil.isNotEmpty(dtSet)) {
          ...
          DtQuery dtQuery = new DtQuery();
          dtQuery.setDtIdAndin(new ArrayList<>(inDtId));
          Collection<DtVO> rDtVOSet = this.queryDtVO(dtQuery);
  ...
  public Collection<DtVO> queryDtVO(DtQuery dtQuery) throws CodePluginException {
    if (log.isDebugEnabled()) {
      log.debug("Staring call DtPersistent.queryDtVO ");
      log.debug("parameter dtQuery is : " + dtQuery);
    }
    try {
      StringBuilder querySql = new StringBuilder(SELECT_VO_SQL);
      ...
      querySql.append(this.getSortSql(SORT, COLUMNS_PARAMETER, TABLE_ALIAS));
      return this.query(querySql, DtVO.class, dtQuery);
    } catch (org.springframework.dao.DataAccessException e) {
      if (log.isErrorEnabled()) {
        log.error(e.getMessage(), e);
      }
  ...
  public Paging<DtVO> pagingQueryDtVO(Parameter parameter, DtQuery dtQuery) throws CodePluginException {
    if (log.isDebugEnabled()) {
      log.debug("Staring call DtPersistent.pagingQueryDtVO ");
      log.debug("parameter parameter is : " + parameter);
      log.debug("parameter dtQuery is : " + dtQuery);
    }
    try {
