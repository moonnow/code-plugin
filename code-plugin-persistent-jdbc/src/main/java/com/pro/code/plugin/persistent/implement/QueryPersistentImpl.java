...
//  }
  ...
  public QueryVO getQueryVOByPk(java.lang.String queryId) throws CodePluginException {
    if (log.isDebugEnabled()) {
      log.debug("Staring call QueryPersistent.getQueryVOByPk ");
      log.debug("parameter queryId is : " + queryId);
    }
    try {
      if (ToolUtil.isNullStr(queryId)) {
  ...
  public Collection<QueryVO> getAllQueryVO() throws CodePluginException {
    if (log.isDebugEnabled()) {
      log.debug("Staring call QueryPersistent.getAllQueryVO ");
    }
    try {
      StringBuilder querySql = new StringBuilder(SELECT_VO_SQL);
      querySql.append(this.getSortSql(SORT, COLUMNS_PARAMETER, TABLE_ALIAS));
  ...
  public Paging<QueryVO> pagingGetQueryVO(Parameter parameter) throws CodePluginException {
    if (log.isDebugEnabled()) {
      log.debug("Staring call QueryPersistent.pagingGetQueryVO ");
      log.debug("parameter parameter is : " + parameter);
    }
    try {
      if (parameter == null) {
      ...
      Paging<QueryVO> paging = new Paging<>(parameter);
      StringBuilder countSql = new StringBuilder(COUNT_SQL);
      Long count = this.queryCount(countSql, Long.class);
      paging.setCount(count);
      if (count > 0) {
        ...
        if (ToolUtil.isNotEmpty(querySet)) {
          ...
          QueryQuery queryQuery = new QueryQuery();
          queryQuery.setQueryIdAndin(new ArrayList<>(inQueryId));
          Collection<QueryVO> rQueryVOSet = this.queryQueryVO(queryQuery);
  ...
  public Collection<QueryVO> queryQueryVO(QueryQuery queryQuery) throws CodePluginException {
    if (log.isDebugEnabled()) {
      log.debug("Staring call QueryPersistent.queryQueryVO ");
      log.debug("parameter queryQuery is : " + queryQuery);
    }
    try {
      StringBuilder querySql = new StringBuilder(SELECT_VO_SQL);
      ...
      querySql.append(this.getSortSql(SORT, COLUMNS_PARAMETER, TABLE_ALIAS));
      return this.query(querySql, QueryVO.class, queryQuery);
    } catch (org.springframework.dao.DataAccessException e) {
      if (log.isErrorEnabled()) {
        log.error(e.getMessage(), e);
      }
  ...
  public Paging<QueryVO> pagingQueryQueryVO(Parameter parameter, QueryQuery queryQuery) throws CodePluginException {
    if (log.isDebugEnabled()) {
      log.debug("Staring call QueryPersistent.pagingQueryQueryVO ");
      log.debug("parameter parameter is : " + parameter);
      log.debug("parameter queryQuery is : " + queryQuery);
    }
    try {
