...
public class SortController extends ToolController {
  ...
  @ResponseBody
  public Responses<SortVO> getVOByPk(Parameter parameter) {
    if (log.isDebugEnabled()) {
      log.debug("Staring call SortController.getVOByPk ");
      log.debug("parameter parameter is : " + parameter);
    }
    ...
    try {
      ...
      responses.setData(codePluginService.getSortVOByPk(parameter.getPrimaryKey()));
    } catch (Exception e) {
      if (log.isErrorEnabled()) {
        log.error(e.getMessage(), e);
      }
  ...
  @ResponseBody
  public Responses<SortVO> getVO(Parameter parameter, @RequestBody SortQuery sortQuery) {
    if (log.isDebugEnabled()) {
      log.debug("Staring call SortController.getVO ");
      log.debug("parameter parameter is : " + parameter);
      log.debug("parameter sortQuery is : " + sortQuery);
    }
    Responses<SortVO> responses = new Responses<>(parameter);
    try {
      if (sortQuery == null || ToolUtil.isNullEntityAllFieldValue(sortQuery)) {
        if (ToolUtil.ACTION_GET_ALL_VO.equals(parameter.getAction())) {
          responses.setData(codePluginService.getAllSortVO());
        } else if (ToolUtil.ACTION_PAGING_VO.equals(parameter.getAction())) {
          responses.setData(codePluginService.pagingGetSortVO(parameter));
        } else {
          throw CodePluginException.getException(CodePluginException.FW_PARAMETER_IS_ERROR, " action ");
        }
      } else {
        if (ToolUtil.ACTION_GET_ALL_VO.equals(parameter.getAction())) {
          responses.setData(codePluginService.querySortVO(sortQuery));
        } else if (ToolUtil.ACTION_PAGING_VO.equals(parameter.getAction())) {
          responses.setData(codePluginService.pagingQuerySortVO(parameter, sortQuery));
        } else {
          throw CodePluginException.getException(CodePluginException.FW_PARAMETER_IS_ERROR, " action ");
        }
      }
