...
public class QueryController extends ToolController {
  ...
  @ResponseBody
  public Responses<QueryVO> getVOByPk(Parameter parameter) {
    if (log.isDebugEnabled()) {
      log.debug("Staring call QueryController.getVOByPk ");
      log.debug("parameter parameter is : " + parameter);
    }
    ...
    try {
      ...
      responses.setData(codePluginService.getQueryVOByPk(parameter.getPrimaryKey()));
    } catch (Exception e) {
      if (log.isErrorEnabled()) {
        log.error(e.getMessage(), e);
      }
  ...
  @ResponseBody
  public Responses<QueryVO> getVO(Parameter parameter, @RequestBody QueryQuery queryQuery) {
    if (log.isDebugEnabled()) {
      log.debug("Staring call QueryController.getVO ");
      log.debug("parameter parameter is : " + parameter);
      log.debug("parameter queryQuery is : " + queryQuery);
    }
    Responses<QueryVO> responses = new Responses<>(parameter);
    try {
      if (queryQuery == null || ToolUtil.isNullEntityAllFieldValue(queryQuery)) {
        if (ToolUtil.ACTION_GET_ALL_VO.equals(parameter.getAction())) {
          responses.setData(codePluginService.getAllQueryVO());
        } else if (ToolUtil.ACTION_PAGING_VO.equals(parameter.getAction())) {
          responses.setData(codePluginService.pagingGetQueryVO(parameter));
        } else {
          throw CodePluginException.getException(CodePluginException.FW_PARAMETER_IS_ERROR, " action ");
        }
      } else {
        if (ToolUtil.ACTION_GET_ALL_VO.equals(parameter.getAction())) {
          responses.setData(codePluginService.queryQueryVO(queryQuery));
        } else if (ToolUtil.ACTION_PAGING_VO.equals(parameter.getAction())) {
          responses.setData(codePluginService.pagingQueryQueryVO(parameter, queryQuery));
        } else {
          throw CodePluginException.getException(CodePluginException.FW_PARAMETER_IS_ERROR, " action ");
        }
      }
