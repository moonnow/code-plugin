...
public class DtController extends ToolController {
  ...
  public Responses<Dt> get(Parameter parameter, @RequestBody DtQuery dtQuery) {
    ...
    Responses<Dt> responses = new Responses<>(parameter);
    try {
  ...
  @ResponseBody
  public Responses<DtVO> getVOByPk(Parameter parameter) {
    if (log.isDebugEnabled()) {
      log.debug("Staring call DtController.getVOByPk ");
      log.debug("parameter parameter is : " + parameter);
    }
    ...
    try {
      ...
      responses.setData(codePluginService.getDtVOByPk(parameter.getPrimaryKey()));
    } catch (Exception e) {
      if (log.isErrorEnabled()) {
        log.error(e.getMessage(), e);
      }
  ...
  @ResponseBody
  public Responses<DtVO> getVO(Parameter parameter, @RequestBody DtQuery dtQuery) {
    if (log.isDebugEnabled()) {
      log.debug("Staring call DtController.getVO ");
      log.debug("parameter parameter is : " + parameter);
      log.debug("parameter dtQuery is : " + dtQuery);
    }
    Responses<DtVO> responses = new Responses<>(parameter);
    try {
      if (dtQuery == null || ToolUtil.isNullEntityAllFieldValue(dtQuery)) {
        if (ToolUtil.ACTION_GET_ALL_VO.equals(parameter.getAction())) {
          responses.setData(codePluginService.getAllDtVO());
        } else if (ToolUtil.ACTION_PAGING_VO.equals(parameter.getAction())) {
          responses.setData(codePluginService.pagingGetDtVO(parameter));
        } else {
          throw CodePluginException.getException(CodePluginException.FW_PARAMETER_IS_ERROR, " action ");
        }
      } else {
        if (ToolUtil.ACTION_GET_ALL_VO.equals(parameter.getAction())) {
          responses.setData(codePluginService.queryDtVO(dtQuery));
        } else if (ToolUtil.ACTION_PAGING_VO.equals(parameter.getAction())) {
          responses.setData(codePluginService.pagingQueryDtVO(parameter, dtQuery));
        } else {
          throw CodePluginException.getException(CodePluginException.FW_PARAMETER_IS_ERROR, " action ");
        }
      }
