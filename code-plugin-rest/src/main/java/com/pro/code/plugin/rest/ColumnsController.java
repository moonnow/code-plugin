...
public class ColumnsController extends ToolController {
  ...
  @ResponseBody
  public Responses<Columns> batchSave(Parameter parameter, @RequestBody List<Columns> columnss) {
    if (log.isDebugEnabled()) {
    ...
    try {
      ...
      codePluginService.batchSaveColumns(columnss);
    } catch (Exception e) {
      if (log.isErrorEnabled()) {
        log.error(e.getMessage(), e);
      }
  ...
  @ResponseBody
  public Responses<Columns> batchUpdate(Parameter parameter, @RequestBody List<Columns> columnss) {
    if (log.isDebugEnabled()) {
    ...
    try {
      ...
      codePluginService.batchUpdateColumns(columnss);
    } catch (Exception e) {
      if (log.isErrorEnabled()) {
        log.error(e.getMessage(), e);
      }
  ...
  @ResponseBody
  public Responses<Columns> batchRemove(Parameter parameter, @RequestBody List<Columns> columnss) {
    if (log.isDebugEnabled()) {
    ...
    try {
      ...
      codePluginService.batchRemoveColumns(columnss);
    } catch (Exception e) {
      if (log.isErrorEnabled()) {
        log.error(e.getMessage(), e);
      }
  ...
  public Responses<Columns> get(Parameter parameter, @RequestBody ColumnsQuery columnsQuery) {
    ...
    Responses<Columns> responses = new Responses<>(parameter);
    try {
  ...
  @ResponseBody
  public Responses<ColumnsVO> getVOByPk(Parameter parameter) {
    if (log.isDebugEnabled()) {
      log.debug("Staring call ColumnsController.getVOByPk ");
      log.debug("parameter parameter is : " + parameter);
    }
    ...
    try {
      ...
      responses.setData(codePluginService.getColumnsVOByPk(parameter.getPrimaryKey()));
    } catch (Exception e) {
      if (log.isErrorEnabled()) {
        log.error(e.getMessage(), e);
      }
  ...
  @ResponseBody
  public Responses<ColumnsVO> getVO(Parameter parameter, @RequestBody ColumnsQuery columnsQuery) {
    if (log.isDebugEnabled()) {
      log.debug("Staring call ColumnsController.getVO ");
      log.debug("parameter parameter is : " + parameter);
      log.debug("parameter columnsQuery is : " + columnsQuery);
    }
    Responses<ColumnsVO> responses = new Responses<>(parameter);
    try {
      if (columnsQuery == null || ToolUtil.isNullEntityAllFieldValue(columnsQuery)) {
        if (ToolUtil.ACTION_GET_ALL_VO.equals(parameter.getAction())) {
          responses.setData(codePluginService.getAllColumnsVO());
        } else if (ToolUtil.ACTION_PAGING_VO.equals(parameter.getAction())) {
          responses.setData(codePluginService.pagingGetColumnsVO(parameter));
        } else {
          throw CodePluginException.getException(CodePluginException.FW_PARAMETER_IS_ERROR, " action ");
        }
      } else {
        if (ToolUtil.ACTION_GET_ALL_VO.equals(parameter.getAction())) {
          responses.setData(codePluginService.queryColumnsVO(columnsQuery));
        } else if (ToolUtil.ACTION_PAGING_VO.equals(parameter.getAction())) {
          responses.setData(codePluginService.pagingQueryColumnsVO(parameter, columnsQuery));
        } else {
          throw CodePluginException.getException(CodePluginException.FW_PARAMETER_IS_ERROR, " action ");
        }
      }
