...
public class PkController extends ToolController {
  ...
  @ResponseBody
  public Responses<PkVO> getVOByPk(Parameter parameter) {
    if (log.isDebugEnabled()) {
      log.debug("Staring call PkController.getVOByPk ");
      log.debug("parameter parameter is : " + parameter);
    }
    ...
    try {
      ...
      responses.setData(codePluginService.getPkVOByPk(parameter.getPrimaryKey()));
    } catch (Exception e) {
      if (log.isErrorEnabled()) {
        log.error(e.getMessage(), e);
      }
  ...
  @ResponseBody
  public Responses<PkVO> getVO(Parameter parameter, @RequestBody PkQuery pkQuery) {
    if (log.isDebugEnabled()) {
      log.debug("Staring call PkController.getVO ");
      log.debug("parameter parameter is : " + parameter);
      log.debug("parameter pkQuery is : " + pkQuery);
    }
    Responses<PkVO> responses = new Responses<>(parameter);
    try {
      if (pkQuery == null || ToolUtil.isNullEntityAllFieldValue(pkQuery)) {
        if (ToolUtil.ACTION_GET_ALL_VO.equals(parameter.getAction())) {
          responses.setData(codePluginService.getAllPkVO());
        } else if (ToolUtil.ACTION_PAGING_VO.equals(parameter.getAction())) {
          responses.setData(codePluginService.pagingGetPkVO(parameter));
        } else {
          throw CodePluginException.getException(CodePluginException.FW_PARAMETER_IS_ERROR, " action ");
        }
      } else {
        if (ToolUtil.ACTION_GET_ALL_VO.equals(parameter.getAction())) {
          responses.setData(codePluginService.queryPkVO(pkQuery));
        } else if (ToolUtil.ACTION_PAGING_VO.equals(parameter.getAction())) {
          responses.setData(codePluginService.pagingQueryPkVO(parameter, pkQuery));
        } else {
          throw CodePluginException.getException(CodePluginException.FW_PARAMETER_IS_ERROR, " action ");
        }
      }
